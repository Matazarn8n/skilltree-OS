...
description: "Empêche tes sources de vérité de diverger entre elles : CRM, tableurs, outils tiers restent synchronisés."
---

# Administrateur de registres · fiche skill SkillTree

**Catégorie :** Back-office · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Empêche tes sources de vérité de diverger entre elles : CRM, tableurs, outils tiers restent synchronisés.

## Clés API requises

Attio, HubSpot (optionnel — dégrade proprement sans clé).
...
- « Lance le Administrateur de registres »
- « Lance le Administrateur de registres · [ton contexte précis ici] »

Ou juste : « Lance le Administrateur de registres » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il traque les écarts entre systèmes avant qu'ils ne deviennent un vrai problème.
...
3. Sauvegarder la sortie dans `outputs/records-administrator/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/records-administrator/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/records-administrator.md), texte entièrement réécrit, jamais copié.*
