---
name: call-prep-researcher
description: "Construit un brief d'une page avant chaque rendez-vous : qui tu as en face, ce que fait son entreprise, tout l'historique de votre relation tiré de ta base de connaissance et de tes échanges passés, les sujets encore ouverts, et un objectif suggéré pour l'appel."
---

# Chercheur de prépa d'appel · fiche skill SkillTree

**Catégorie :** Affaires · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Construit un brief d'une page avant chaque rendez-vous : qui tu as en face, ce que fait son entreprise, tout l'historique de votre relation tiré de ta base de connaissance et de tes échanges passés, les sujets encore ouverts, et un objectif suggéré pour l'appel.

## Clés API requises

Cal (optionnel — dégrade proprement sans clé).
...
- « Lance le Chercheur de prépa d'appel »
- « Lance le Chercheur de prépa d'appel · [ton contexte précis ici] »

Ou juste : « Lance le Chercheur de prépa d'appel » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il arrive à l'appel préparé comme si tu avais passé une heure à te documenter.
...
3. Sauvegarder la sortie dans `outputs/call-prep-researcher/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/call-prep-researcher/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/call-prep-researcher.md), texte entièrement réécrit, jamais copié.*
