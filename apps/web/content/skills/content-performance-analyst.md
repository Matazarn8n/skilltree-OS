...
description: "Transforme ton propre historique de publication en stratégie de contenu : ce qui a marché, ce qui n'a pas marché, pourquoi."
---

# Analyste de performance de contenu · fiche skill SkillTree

**Catégorie :** Marketing · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Transforme ton propre historique de publication en stratégie de contenu : ce qui a marché, ce qui n'a pas marché, pourquoi.

## Clés API requises

Apify (optionnel — dégrade proprement sans clé).
...
- « Lance le Analyste de performance de contenu »
- « Lance le Analyste de performance de contenu · [ton contexte précis ici] »

Ou juste : « Lance le Analyste de performance de contenu » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il tire les leçons de tes propres publications avant de proposer un nouveau format.
...
3. Sauvegarder la sortie dans `outputs/content-performance-analyst/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/content-performance-analyst/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/content-performance-analyst.md), texte entièrement réécrit, jamais copié.*
