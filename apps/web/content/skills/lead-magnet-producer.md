...
description: "Transforme un contenu qui a déjà performé en actif à formulaire qui capture la demande qu'il a prouvée."
---

# Producteur de lead magnets · fiche skill SkillTree

**Catégorie :** Marketing · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Transforme un contenu qui a déjà performé en actif à formulaire qui capture la demande qu'il a prouvée.

## Clés API requises

Beehiiv, Resend (optionnel — dégrade proprement sans clé).
...
- « Lance le Producteur de lead magnets »
- « Lance le Producteur de lead magnets · [ton contexte précis ici] »

Ou juste : « Lance le Producteur de lead magnets » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il recycle ce qui a déjà prouvé sa valeur plutôt que de partir de zéro.
...
3. Sauvegarder la sortie dans `outputs/lead-magnet-producer/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/lead-magnet-producer/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/lead-magnet-producer.md), texte entièrement réécrit, jamais copié.*
