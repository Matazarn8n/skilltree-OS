---
name: client-portal-manager
description: "Donne à chaque client un endroit unique pour voir son projet avancer : ce qui est livré, ce qui est en cours, ce qu'il doit fournir, et ce qui arrive ensuite."
---

# Gestionnaire de portail client · fiche skill SkillTree

**Catégorie :** Opérations · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Donne à chaque client un endroit unique pour voir son projet avancer : ce qui est livré, ce qui est en cours, ce qu'il doit fournir, et ce qui arrive ensuite.
...
- « Lance le Gestionnaire de portail client »
- « Lance le Gestionnaire de portail client · [ton contexte précis ici] »

Ou juste : « Lance le Gestionnaire de portail client » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il préfère qu'un client voie l'avancement plutôt qu'il ait à le demander.
...
3. Sauvegarder la sortie dans `outputs/client-portal-manager/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/client-portal-manager/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/client-portal-manager.md), texte entièrement réécrit, jamais copié.*
