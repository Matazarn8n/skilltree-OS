---
name: market-mapper
description: "Fait le recensement de toutes les entreprises qui correspondent à ton ICP sur un vertical cible — avant même qu'un seul email ne soit écrit."
---

# Cartographe de marché · fiche skill SkillTree

**Catégorie :** Ventes · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Fait le recensement de toutes les entreprises qui correspondent à ton ICP sur un vertical cible — avant même qu'un seul email ne soit écrit.
...
- « Lance le Cartographe de marché »
- « Lance le Cartographe de marché · [ton contexte précis ici] »

Ou juste : « Lance le Cartographe de marché » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il cartographie le marché entier avant de prioriser une seule cible.
...
3. Sauvegarder la sortie dans `outputs/market-mapper/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/market-mapper/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/market-mapper.md), texte entièrement réécrit, jamais copié.*
