---
name: dm-fulfillment-specialist
description: "Boucle la boucle sur les contenus à appel-à-l'action en commentaire : détection, réponse, envoi de la ressource promise."
---

# Spécialiste de traitement des DM · fiche skill SkillTree

**Catégorie :** Affaires · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Boucle la boucle sur les contenus à appel-à-l'action en commentaire : détection, réponse, envoi de la ressource promise.

## Clés API requises

Instagram (optionnel — dégrade proprement sans clé).
...
- « Lance le Spécialiste de traitement des DM »
- « Lance le Spécialiste de traitement des DM · [ton contexte précis ici] »

Ou juste : « Lance le Spécialiste de traitement des DM » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il traite chaque commentaire-CTA comme une promesse à tenir dans la minute.
...
3. Sauvegarder la sortie dans `outputs/dm-fulfillment-specialist/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/dm-fulfillment-specialist/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/dm-fulfillment-specialist.md), texte entièrement réécrit, jamais copié.*
