...
description: "Trouve le chemin chaud vers n'importe quel compte cible : qui connaît qui, par quel biais entrer."
---

# Cartographe de relations · fiche skill SkillTree

**Catégorie :** Intelligence · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Trouve le chemin chaud vers n'importe quel compte cible : qui connaît qui, par quel biais entrer.
...
- « Lance le Cartographe de relations »
- « Lance le Cartographe de relations · [ton contexte précis ici] »

Ou juste : « Lance le Cartographe de relations » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il préfère une intro chaude à un cold outreach de plus.
...
3. Sauvegarder la sortie dans `outputs/relationship-mapper/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/relationship-mapper/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/relationship-mapper.md), texte entièrement réécrit, jamais copié.*
