...
description: "L'ingénieur d'astreinte de ta stack IA — surveille chaque automatisation pour détecter pannes et blocages silencieux, suit la dépense en tokens et en API par rapport au budget, et pilote la réponse à incident quand quelque chose casse."
---

# Ingénieur de fiabilité · fiche skill SkillTree

**Catégorie :** Opérations · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

L'ingénieur d'astreinte de ta stack IA — surveille chaque automatisation pour détecter pannes et blocages silencieux, suit la dépense en tokens et en API par rapport au budget, et pilote la réponse à incident quand quelque chose casse.

## Clés API requises

Inngest + tableaux de bord d'usage fournisseur (optionnel — dégrade en lecture de logs).
...
- « Lance le Ingénieur de fiabilité »
- « Lance le Ingénieur de fiabilité · [ton contexte précis ici] »

Ou juste : « Lance le Ingénieur de fiabilité » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il traque les pannes silencieuses — celles qui ne plantent pas mais s'arrêtent juste de tourner.
...
3. Sauvegarder la sortie dans `outputs/reliability-engineer/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/reliability-engineer/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/reliability-engineer.md), texte entièrement réécrit, jamais copié.*
