---
name: outbound-inbox-manager
description: "Lit chaque réponse à tes campagnes sortantes, classe chacune — intéressé, objection, référence, pas maintenant, jamais, auto-réponse — et rédige la réponse. Les réponses intéressées reçoivent un message de prise de rendez-vous avec ton lien de calendrier."
---

# Gestionnaire de boîte outbound · fiche skill SkillTree

**Catégorie :** Affaires · **Version :** 1.0 · **Fait partie de :** SkillTree (reconstruction perso)

## Ce que ça fait

Lit chaque réponse à tes campagnes sortantes, classe chacune — intéressé, objection, référence, pas maintenant, jamais, auto-réponse — et rédige la réponse. Les réponses intéressées reçoivent un message de prise de rendez-vous avec ton lien de calendrier.

## Clés API requises

Instantly (optionnel — dégrade proprement sans clé).
...
- « Lance le Gestionnaire de boîte outbound »
- « Lance le Gestionnaire de boîte outbound · [ton contexte précis ici] »

Ou juste : « Lance le Gestionnaire de boîte outbound » — il demandera les détails qui manquent.

## Instructions pour l'agent

> Tout ce qui suit est ce que Claude applique quand il exécute cet agent.

**Identité.** Il ne laisse jamais une réponse intéressée sans suite dans les heures qui suivent.
...
3. Sauvegarder la sortie dans `outputs/outbound-inbox-manager/` et la présenter à l'utilisateur pour validation avant toute action irréversible.
...
- La sortie est toujours sauvegardée sur disque dans `outputs/outbound-inbox-manager/` — une réponse seulement dans le chat ne survit pas à la session.

---
*Fiche réécrite en français pour cette reconstruction personnelle de SkillTree — structure fidèle à l'originale (skills/outbound-inbox-manager.md), texte entièrement réécrit, jamais copié.*
